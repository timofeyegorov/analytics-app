...
def calculate_traffic_sources(df):
    ...
    output_df.insert(0, 'Источник', 0)  # Столбец названия подкатегорий указанной категории
    ...
    for i, subcategory_name in enumerate(created_columns):
        ...
        if temp_df.shape[0] != 0:
            ...
            output_df.loc[i, 'Цена ЦА 5-6'] = temp_df[temp_df['target_class'].isin([5, 6])]['channel_expense'].sum() / \
                                                temp_df[temp_df['target_class'].isin([5, 6])].shape[0] if \
                                                temp_df[temp_df['target_class'].isin([5, 6])].shape[0] != 0 else 0
            output_df.loc[i, 'Цена ЦА 4-5-6'] = temp_df[temp_df['target_class'].isin([4, 5, 6])][
                                                      'channel_expense'].sum() / \
                                                  temp_df[temp_df['target_class'].isin([4, 5, 6])].shape[0] if \
