...
def calculate_traffic_sources(df):
    ...
    payment_status = status[status['Оплатил'] == ' +'][
        'Статус АМО'].unique().tolist()  # Статусы с этапом "оплатил"
    conversation_status = status[status['Был разговор'] == ' +'][
        'Статус АМО'].unique().tolist()  # Статусы с этапом "был разговор"
    ...
    for i, subcategory_name in enumerate(created_columns):
        ...
        if temp_df.shape[0] != 0:
            ...
            df_category.loc[i, '% в работе'] = round(temp_df[temp_df['status_amo'].isin(progress_status)].shape[0]\
                                                     / temp_df.shape[0] * 100, 1)
            df_category.loc[i, '% дозвонов'] = round(temp_df[temp_df['status_amo'].isin(conversation_status)].shape[0] \
                                                     / temp_df.shape[0] * 100, 1)
            df_category.loc[i, '% офферов'] = round(temp_df[temp_df['status_amo'] == 'Сделан оффер из Теплые продажи'].shape[0] \
                                                    / temp_df.shape[0] * 100, 1)
            df_category.loc[i, '% счетов'] = round(temp_df[temp_df['status_amo'] == 'Выставлен счет из Теплые продажи'].shape[0] \
                                                   / temp_df.shape[0] * 100, 1)
            df_category.loc[i, 'Цена лида'] = round(temp_df[temp_df['channel_expense'] != 0]['channel_expense'].sum() \
                                                    / temp_df.shape[0], 1)

            df_category.loc[i, '% ЦА 5-6'] = round(temp_df[temp_df['target_class'].isin([5, 6])].shape[0] \
                                                   / temp_df.shape[0] * 100, 1)
            df_category.loc[i, '% ЦА 5'] = round(temp_df[temp_df['target_class'] == 5].shape[0] \
                                                 / temp_df.shape[0] * 100, 1)
            df_category.loc[i, '% ЦА 6'] = round(temp_df[temp_df['target_class'] == 6].shape[0] \
                                                 / temp_df.shape[0] * 100, 1)
            df_category.loc[i, '% ЦА 4-5-6'] = round(temp_df[temp_df['target_class'].isin([4, 5, 6])].shape[0] \
                                                     / temp_df.shape[0] * 100, 1)
            df_category.loc[i, '% ЦА 4'] = round(temp_df[temp_df['target_class'] == 4].shape[0] \
                                                 / temp_df.shape[0] * 100, 1)
            df_category.loc[i, '% не ЦА'] = round(temp_df[temp_df['target_class'] == 0].shape[0] \
                                                  / temp_df.shape[0] * 100, 1)
            df_category.loc[i, '% ЦА 3'] = round(temp_df[temp_df['target_class'] == 3].shape[0] \
                                                 / temp_df.shape[0] * 100, 1)
            df_category.loc[i, '% ЦА 2'] = round(temp_df[temp_df['target_class'] == 2].shape[0] \
                                                 / temp_df.shape[0] * 100, 1)
            df_category.loc[i, '% ЦА 1'] = round(temp_df[temp_df['target_class'] == 1].shape[0] \
                                                 / temp_df.shape[0] * 100, 1)
            ...
            df_category.loc[i, '% продаж'] = round(temp_df[temp_df['status_amo'].isin(payment_status)].shape[0] \
                                            / temp_df.shape[0] * 100, 1)
            ...
            df_category.loc[i, 'Расход на ОП'] = (temp_df['payment_amount'].sum() * 0.06 + 150 *
            ...
            df_category.loc[i, 'Расход общий (ОП+бюджет)'] = df_category.loc[i, 'Бюджет'] + df_category.loc[i, 'Расход на ОП']
            df_category.loc[i, '% расходов на трафик (доля от Расход общий)'] = round(df_category.loc[i, 'Бюджет'] / df_category.loc[i, 'Расход общий (ОП+бюджет)'] * 100, 1)
            ...
            df_category.loc[i, 'ROI'] = round((temp_df['payment_amount'].sum() - temp_df['channel_expense'].sum()) / temp_df[
                                         'channel_expense'].sum() * 100, 1) if temp_df[temp_df['payment_amount'] != 0].shape[0] != 0 else 0
            ...
            df_category.loc[i, 'Оборот на лида (на обработанного лида)'] = df_category.loc[i, 'Оборот'] / temp_df[temp_df['status_amo'] == 'Обработанная заявка из Теплые продажи'].shape[0]
            df_category.loc[i, 'Оборот на разговор'] = df_category.loc[i, 'Оборот'] / temp_df[temp_df['status_amo'].isin(conversation_status)].shape[0]
            ...
            df_category.loc[i, 'CV разговор/оплата'] = \
                                                    ...
                                                    temp_df[temp_df['payment_amount'] != 0].shape[0] \
                                                        if temp_df[temp_df['payment_amount'] != 0].shape[0] != 0 else 0
            df_category.loc[i, 'CPO'] = round(
                                        df_category.loc[i, 'Расход общий (ОП+бюджет)'] / temp_df[temp_df['payment_amount'] != 0].shape[0], 1) \
                                        if temp_df[temp_df['payment_amount'] != 0].shape[0] != 0 else 0
            df_category.loc[i, '% лидов (доля от общего)'] = round(temp_df.shape[0] / df.shape[0] * 100, 1)
            df_category.loc[i, '% Оборот (доля от общего)'] = round(
                                                      temp_df['payment_amount'].sum() / df['payment_amount'].sum(), 1)
            df_category.loc[i, '% Оплат (доля от общего)'] = round(
                                                    temp_df[temp_df['payment_amount'].isin(payment_status)].shape[0] /
                                                    df[df['payment_amount'].isin(payment_status)].shape[0], 1) \
                                                    if df[df['payment_amount'].isin(payment_status)].shape[0] != 0 else 0

            df_category.loc[i, '% Затрат (доля от общего)'] = \
                        round(float(df_category.loc[i, 'Бюджет']) + float(df_category.loc[i, 'Расход на ОП']) / \
                              (float(df['channel_expense'].sum()) + \
                               ((float(df['payment_amount'].sum()) * 0.06 + 150 * float(
                                   df[df['payment_amount'] != 0].shape[0]) + 27 * float(
                                   df[df['payment_amount'] != 0].shape[0]) + 20 * float(
                                   df[df['payment_amount'] != 0].shape[0])) * 1.4)) * 100, 1) if (
                                    float(df['channel_expense'].sum()) + \
