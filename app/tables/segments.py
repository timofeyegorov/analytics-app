...
def calculate_segments(df):
  ...
  traff_data = pd.read_csv(os.path.join(RESULTS_FOLDER, 'trafficologists.csv'))
  ...
  def channels_table(df, column_name):
    ...
    for i in range(1, df_category_val.shape[1]):
      df_category_val.iloc[idx+1, i] = df_category_val[df_category_val[column_name].isin(target_audience)].iloc[:,i].sum()
      df_category.iloc[idx+1, i] = df_category[df_category[column_name].isin(target_audience)].iloc[:,i].sum() 
  ...
  def target_consolidated(df):
    ...
    for idx, column_category in enumerate(df.columns.values[2:8]):
      ...
      for traff_name in created_columns:
        df_segment_val.loc[idx, traff_name] = df[df[column_category].isin(target_audience) & ((df['trafficologist']==traff_name) | (df['account']==traff_name))].shape[0]
        df_segment.loc[idx, traff_name] = round(df[df[column_category].isin(target_audience) & ((df['trafficologist']==traff_name) | (df['account']==traff_name))].shape[0]/\
