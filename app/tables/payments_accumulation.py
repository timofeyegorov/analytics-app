...
def calculate_payments_accumulation(payments_df):
    ...
    for cur_order_date in unique_order_dates:
        ...
        for cur_payment_date in temp_unique_payment_dates:
            temp_vals[cur_payment_date.week+1] += filtered_payments_df[filtered_payments_df['Дата оплаты'] == cur_payment_date]['Сумма оплаты'].sum()
    res_df = pd.DataFrame(columns=['Месяц', 'Диапазон'] + [i for i in range(1, 54)], data=values)
