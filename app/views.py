...
class ZoomsView(FilteringBaseView):
    ...
    def __check_date(current_date: str, date_from: datetime, date_to: datetime):
        if date_to is None:
            return date_from <= datetime.datetime.strptime(current_date, "%Y%m%d").date()
        return date_from <= datetime.datetime.strptime(current_date, "%Y%m%d").date() <= date_to

    def __read_s3_json_to_df(self, json_name: str,
        ...
        for folder_path in s3.get_paths_2_level():
            ...
            if (manager in manager_list) and self.__check_date(current_date=date_str,
                                                               date_from=date_from,
                                                               date_to=date_to):
                try:
...
class IntensivesEvents(TemplateView):
    ...
    def post(self):
        try:
            # Даты мероприятий
            start_date_event = datetime.datetime.strptime(
                request.form["start_date"], "%Y-%m-%d"
            ).date()
            end_date_event = datetime.datetime.strptime(
                request.form["end_date"], "%Y-%m-%d"
            ).date()
            # Даты оплаты
            ...
            if custom_period:
                ...
                start_date_pay = datetime.datetime.strptime(
                    start_date_custom, "%Y-%m-%d"
                ).date()
                end_date_pay = datetime.datetime.strptime(
                    end_date_custom, "%Y-%m-%d"
                ).date()
            elif checkbox_value:
                checkbox_values = {
                    "1week": 7,
                ...
                end_date_pay = (
                        end_date_event
                        + datetime.timedelta(
                    days=checkbox_values.get(checkbox_value)
                )
                ).strftime("%Y-%m-%d")
                start_date_pay = end_date_event.strftime("%Y-%m-%d")
            get_payment(start_date_pay, end_date_pay)
            table = get_funnel_payment(
                start_date_event.strftime("%Y-%m-%d"),
                end_date_event.strftime("%Y-%m-%d"),
            )
