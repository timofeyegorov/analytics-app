...
def calculate(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    for col in columns:
        if col not in [
            ...
            output_df.loc[7, "% в работе"] = round(
                df[
                    (df["target_class"].isin([5, 6]))
                    & (df["status_amo"].isin(progress_status))
                ].shape[0]
                / df[df["target_class"].isin([5, 6])].shape[0]
                * 100,
                1,
            )
            output_df.loc[8, "% в работе"] = round(
                df[df["status_amo"].isin(progress_status)].shape[0] / df.shape[0] * 100,
                1,
            )

        elif col == "% дозвонов":
            output_df.loc[7, "% дозвонов"] = round(
                df[
                    (df["target_class"].isin([5, 6]))
                    & (df["status_amo"].isin(conversation_status))
                ].shape[0]
                / df[df["target_class"].isin([5, 6])].shape[0]
                * 100,
                1,
            )
            output_df.loc[8, "% дозвонов"] = round(
                df[df["status_amo"].isin(conversation_status)].shape[0]
                / df.shape[0]
                * 100,
                1,
            )
            ...
            output_df.loc[7, "% офферов"] = round(
                df[
                    (df["target_class"].isin([5, 6]))
                    & (df["status_amo"].isin(offer_status))
                ].shape[0]
                / df[df["target_class"].isin([5, 6])].shape[0]
                * 100,
                1,
            )
            output_df.loc[8, "% офферов"] = round(
                df[df["status_amo"].isin(offer_status)].shape[0] / df.shape[0] * 100, 1
            )

        elif col == "% счетов":
            output_df.loc[7, "% счетов"] = round(
                df[
                    (df["target_class"].isin([5, 6]))
                    & (df["status_amo"].isin(bill_status))
                ].shape[0]
                / df[df["target_class"].isin([5, 6])].shape[0]
                * 100,
                1,
            )
            output_df.loc[8, "% счетов"] = round(
                df[df["status_amo"].isin(bill_status)].shape[0] / df.shape[0] * 100, 1
            )

        elif col == "% продаж":
            output_df.loc[7, "% продаж"] = round(
                df[
                    (df["target_class"].isin([5, 6]))
                    & (df["status_amo"].isin(payment_status))
                ].shape[0]
                / df[df["target_class"].isin([5, 6])].shape[0]
                * 100,
                1,
            )
            output_df.loc[8, "% продаж"] = round(
                df[df["status_amo"].isin(payment_status)].shape[0] / df.shape[0] * 100,
                1,
            )
        elif col == "Цена лида":
            output_df.loc[7, "Цена лида"] = round(
                df[df["target_class"].isin([5, 6])]["channel_expense"].sum()
                / df[df["target_class"].isin([5, 6])].shape[0],
                1,
            )
            output_df.loc[8, "Цена лида"] = round(
                df["channel_expense"].sum() / df.shape[0], 1
            )

        elif col == "Средний чек":
            output_df.loc[7, "Средний чек"] = (
                round(
                    df[df["target_class"].isin([5, 6])]["payment_amount"].sum()
                    / df[
                        (df["target_class"].isin([5, 6])) & (df["payment_amount"] != 0)
                    ].shape[0],
                    1,
                )
                if df[
            ...
            output_df.loc[8, "Средний чек"] = (
                round(
                    df["payment_amount"].sum() / df[df["payment_amount"] != 0].shape[0],
                    1,
                )
            ...
            output_df.loc[7, col] = (
                round(
                    float(output_df.loc[7, "Бюджет"])
                    / output_df.loc[7, "Расход общий (ОП+бюджет)"]
                    * 100,
                    1,
                )
                if output_df.loc[7, "Расход общий (ОП+бюджет)"] != 0
                else 0
            )
            output_df.loc[8, col] = round(
                float(output_df.loc[8, "Бюджет"])
                / output_df.loc[8, "Расход общий (ОП+бюджет)"]
                * 100,
                1,
            )

        elif col == "% расходов на ОП (доля от Расход общий)":
            output_df.loc[7, col] = round(
                float(output_df.loc[7, "Расход на ОП"])
                / output_df.loc[7, "Расход общий (ОП+бюджет)"]
                * 100,
                1,
            )
            output_df.loc[8, col] = round(
                float(output_df.loc[8, "Расход на ОП"])
                / output_df.loc[8, "Расход общий (ОП+бюджет)"]
                * 100,
                1,
            )
            ...
            output_df.loc[7, "ROI ?"] = (
                ...
                if df[df["target_class"].isin([5, 6])]["channel_expense"].sum() != 0
                else 0
            )
            output_df.loc[7, "ROMI"] = round(
                (output_df["Оборот"][5:7].sum() / output_df["Бюджет"][5:7].sum()) - 1, 1
            )
            output_df.loc[7, "ROSI"] = round(
                (output_df["Оборот"][5:7].sum() / output_df["Расход на ОП"][5:7].sum())
                - 1,
                1,
            )
            output_df.loc[7, "ROI"] = round(
                (
                    output_df["Оборот"][5:7].sum()
                    / output_df["Расход общий (ОП+бюджет)"][5:7].sum()
                )
                - 1,
                1,
            )
            ...
            output_df.loc[8, "ROMI"] = round(
                (output_df["Оборот"][0:7].sum() / output_df["Бюджет"][0:7].sum()) - 1, 1
            )
            output_df.loc[8, "ROSI"] = round(
                (output_df["Оборот"][0:7].sum() / output_df["Расход на ОП"][0:7].sum())
                - 1,
                1,
            )
            output_df.loc[8, "ROI"] = round(
                (
                    output_df["Оборот"][0:7].sum()
                    / output_df["Расход общий (ОП+бюджет)"][0:7].sum()
                )
                - 1,
                1,
            )
            ...
            output_df.loc[7, "Маржа %"] = (
                float(output_df["Маржа"][5:7].sum())
                / float(output_df["Оборот"][5:7].sum())
                if output_df["Оборот"][5:7].sum() != 0
                else 0
            )
            output_df.loc[8, "Маржа %"] = (
                float(output_df["Маржа"][:7].sum())
                / float(output_df["Оборот"][:7].sum())
                if output_df["Оборот"][:7].sum() != 0
                else 0
            )
        elif col == "Цена Разговора":
            output_df.loc[7, "Цена Разговора"] = (
                round(
                    df[df["channel_expense"].isin([5, 6])].sum()
                    / df[
                        (df["status_amo"].isin([5, 6]))
                        & (df["status_amo"].isin(conversation_status))
                    ].shape[0],
                    1,
                )
            ...
            output_df.loc[8, "Цена Разговора"] = (
                round(
                    df[df["channel_expense"].isin([5, 6])].sum()
                    / df[
                        (df["status_amo"].isin([5, 6]))
            ...
            output_df.loc[7, "Цена Оффера"] = (
                round(
                    df[df["channel_expense"].isin([5, 6])].sum()
                    / df[
                        (df["channel_expense"].isin([5, 6]))
                        & (df["status_amo"].isin(offer_status))
                    ].shape[0],
                    1,
                )
            ...
            output_df.loc[8, "Цена Оффера"] = (
                round(
                    df["channel_expense"].sum()
                    / df[df["status_amo"].isin(offer_status)].shape[0],
                    1,
                )
                if df[df["status_amo"].isin(offer_status)].shape[0] != 0
                else 0
            )
        elif col == "Цена Счета":
            output_df.loc[i, "Цена Счета"] = (
            ...
            # elif col == 'Оборот на лида (на обработанного лида)':
            #     pass
            # elif col == 'Оборот на разговор':
            #     pass
            # elif col == 'CV обр.лид/оплата':
            #     pass
            # elif col == 'CV разговор/оплата':
            #     pass
            # elif col == 'CPO':
            #     pass
            # elif col == '% лидов (доля от общего)':
            #     pass
            # elif col == '% Оборот (доля от общего)':
            #     pass
            # elif col == '% Оплат (доля от общего)':
            #     pass
            # elif col == '% Затрат (доля от общего)':
            #     pass
            """
