...
def channels_table(
    ...
    """
    Функция формирования двух датасетов с количеством лидов и процентным соотношением по выбранной категории с разбивкой по трафикологам и их кабинетам
    Вход:
    ...
    data_category = numpy.zeros(
        (len(df[column_name].unique()) + 1, len(created_columns))
    ).astype("int")
    template_df = pandas.DataFrame(data_category, columns=created_columns)
    ...
    for idx, subcategory_name in enumerate(subcategories):
        ...
        df_category.iloc[idx, 1] = int(
            round(
                (df[df[column_name] == subcategory_name].shape[0] / df.shape[0]) * 100,
                0,
            )
        )
        ...
        for traff_name in created_columns:
            df_category_val.loc[idx, traff_name] = df[
                ...
                & ((df["trafficologist"] == traff_name) | (df["account"] == traff_name))
            ].shape[0]
    ...
    for i in range(1, df_category_val.shape[1]):
        ...
        df_category.iloc[idx + 1, i] = (
            df_category[df_category[column_name].isin(target_audience)].iloc[:, i].sum()
        )

    return df_category_val, df_category


def target_audience_channels_table(
    df: pandas.DataFrame, created_columns: List[str]
) -> Tuple[pandas.DataFrame, pandas.DataFrame]:
    """
    Функция формирования двух датасетов по ЦА
    """
    # Формируем заготовки под результирующие датасеты из нулей
    data_ta = numpy.zeros((8, len(created_columns))).astype("int")
    template_df = pandas.DataFrame(data_ta, columns=created_columns)
...
def target_consolidated(
    df: pandas.DataFrame, created_columns: List[str], target_audience: List[str]
) -> Tuple[pandas.DataFrame, pandas.DataFrame]:
    """
    Функция формирования датасетов по ЦА
    """
    # Формируем заготовки под результирующие датасеты из нулей
    data_segment = numpy.zeros((6, len(created_columns))).astype("int")
    template_df = pandas.DataFrame(data_segment, columns=created_columns)
    ...
    for idx, column_category in enumerate(df.columns.values[2:8]):
        ...
        df_segment.loc[idx, "Все"] = round(
            df[df[column_category].isin(target_audience)].shape[0] / df.shape[0] * 100,
            0,
        )
...
def calculate(leads: pandas.DataFrame) -> Dict[str, pandas.DataFrame]:
    pickle_app = current_app.app("pickle")

    target_audience = pickle_app.load("target_audience")
    trafficologists = pickle_app.load("trafficologists")
    ...
    for name in filtered_trafficologists:
        ...
        if trafficologists[trafficologists["name"] == name]["title"].shape[0] > 1:
            created_columns.extend(
                list(leads[leads["trafficologist"] == name]["account"].unique())
            )
    created_columns.sort()

    columns = leads.columns

    df_countries_val, df_countries = channels_table(
        leads, columns[2], created_columns, target_audience
    )
    df_ages_val, df_ages = channels_table(
        leads, columns[3], created_columns, target_audience
    )
    df_jobs_val, df_jobs = channels_table(
        leads, columns[4], created_columns, target_audience
    )
    df_earnings_val, df_earnings = channels_table(
        leads, columns[5], created_columns, target_audience
    )
    df_trainings_val, df_trainings = channels_table(
        leads, columns[6], created_columns, target_audience
    )
    df_times_val, df_times = channels_table(
        leads, columns[7], created_columns, target_audience
    )
    df_target_audience_val, df_target_audience = target_audience_channels_table(
        leads, created_columns
    )
    df_segment_val, df_segment = target_consolidated(
        leads, created_columns, target_audience
    )
