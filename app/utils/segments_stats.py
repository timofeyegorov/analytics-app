...
def calculate(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    def channels_table(df, column_name):
        ...
        if column_name == "Сегменты":
            ...
            data_category = np.zeros(
                (len(df[column_name].unique()), len(columns))
            ).astype("int")
            output_df = pd.DataFrame(
                data_category, columns=columns
            )  # Датасет для процентного кол-ва лидов
            ...
            if column_name == df.columns[3]:
                ...
                subcategories = (
                    temp_list
                    + df[~df[column_name].isin(temp_list)][column_name]
                    .unique()
                    .tolist()
                )
        ...
        for i, subcategory_name in enumerate(subcategories):
            ...
            if temp_df.shape[0] != 0:
                ...
                output_df.loc[i, "% ЦА 5"] = round(
                    temp_df[temp_df["target_class"] == 5].shape[0]
                    / temp_df.shape[0]
                    * 100,
                    1,
                )
                output_df.loc[i, "% ЦА 6"] = round(
                    temp_df[temp_df["target_class"] == 6].shape[0]
                    / temp_df.shape[0]
                    * 100,
                    1,
                )
                ...
                output_df.loc[i, "% ЦА 4"] = round(
                    temp_df[temp_df["target_class"] == 4].shape[0]
                    / temp_df.shape[0]
                    * 100,
                    1,
                )
                output_df.loc[i, "% не ЦА"] = round(
                    temp_df[temp_df["target_class"] == 0].shape[0]
                    / temp_df.shape[0]
                    * 100,
                    1,
                )
                output_df.loc[i, "% ЦА 3"] = round(
                    temp_df[temp_df["target_class"] == 3].shape[0]
                    / temp_df.shape[0]
                    * 100,
                    1,
                )
                output_df.loc[i, "% ЦА 2"] = round(
                    temp_df[temp_df["target_class"] == 2].shape[0]
                    / temp_df.shape[0]
                    * 100,
                    1,
                )
                output_df.loc[i, "% ЦА 1"] = round(
                    temp_df[temp_df["target_class"] == 1].shape[0]
                    / temp_df.shape[0]
                    * 100,
                    1,
                )
                ...
                output_df.loc[i, "Цена ЦА 5-6"] = (
                    round(
                        temp_df[temp_df["target_class"].isin([5, 6])][
                            "channel_expense"
                        ].sum()
                        / temp_df[temp_df["target_class"].isin([5, 6])].shape[0],
                        1,
                ...
                output_df.loc[i, "Расход на ОП"] = round(
                    ...
                    + 20
                    * float(temp_df[temp_df["payment_amount"] != 0].shape[0])
                    * 1.4,
                    1,
                )
                ...
                output_df.loc[i, "ROSI"] = (
                    round(
                        (output_df.loc[i, "Оборот"] / output_df.loc[i, "Расход на ОП"])
                        - 1,
                        1,
                    )
                ...
                output_df.loc[i, "Цена Разговора"] = (
                    round(
                        ...
                        / temp_df[
                            temp_df["status_amo"].isin(conversation_status)
                        ].shape[0],
                        1,
                    )
                        ...
                        / (
                            ...
                            + (
                                (
                                    ...
                                    + 150
                                    * float(df[df["payment_amount"] != 0].shape[0])
    ...
    return {
        "Страны, абсолютные значения": df_countries,
