...
class ApiZoomS3UploadView(APIView):
    ...
    def post(self):
        ...
        managers_zooms['datetime'] = pd.to_datetime(managers_zooms.date) + pd.to_timedelta(managers_zooms.time.astype(str))
        managers_zooms = managers_zooms[(managers_zooms.manager == manager)].loc[:, 'datetime']
            ...
            for path, list_files in request.files.lists():
                ...
                if self.check_date_format(data_in_path, date_format):
                    ...
                    for zt_base, zt_start, zt_end in zoom_timeframes:
                        if (zt_start < datetime64_obj < zt_end):
