class JsonParseService:
    ...
    def to_dict(self, tmp_json: dict) -> dict:
        ...
        data['duration'] = [
            self.minutes_to_hours(tmp_json.get('duration', None))
        ]
        data['text'] = [tmp_json.get('text_type', '').replace("\n", "<br/>")]

        # first hierarchy (total info)
        total_reports = tmp_json.get('content', {}).get('8', {}).get('Отчеты', {})
        ...
        revealing = tmp_json.get('content', {}).get('4', {}).get('Выявление', {})
        ...
        presentation = tmp_json.get('content', {}).get('5', {}).get('Презентация', {})
        ...
        objections = tmp_json.get('content', {}).get('6', {}).get('Возражения', {})
        ...
        materials = tmp_json.get('content', {}).get('7', {}).get('Материалы', {})
