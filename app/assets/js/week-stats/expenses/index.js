    ...
    $(() => {
        ...
        let recalculate = () => {
            let table = $("#statistics > table"),
                ...
                td_expenses = td.filter((_, item) => item.cellIndex === 3).slice(1),
                td_expenses_total = td.filter((_, item) => item.cellIndex === 3)[0],
            ...
            td_expenses.each((_, item) => {
                $(item).text(render_int(item.dataset.value));
            });
            td_expenses_total.dataset.value = td_expenses.map((_, item) => item.dataset.value).toArray().sum();
            $(td_expenses_total).text(render_int(td_expenses_total.dataset.value));

            td_profit.each((_, item) => {
                let row = $(item.parentElement).children("td").slice(4),
                    row_td_expenses = $(item.parentElement).children("td")[2],
                    divider = parseInt(is_profit ? row_td_expenses.dataset.value / 100 : 1),
            ...
            $(td_profit_total).text(render_int(parseInt(td_profit_total.dataset.value) / parseInt(is_profit ? td_expenses_total.dataset.value / 100 : 1)));

            td_values.each((_, item) => {
                let row_td_expenses = $(item.parentElement).children("td")[2],
                    divider = parseInt(is_profit ? row_td_expenses.dataset.value / 100 : 1),
            ...
            td_total.each((_, item) => {
                let col = td.filter((_, td) => td.cellIndex >= (is_accumulative ? 5 : item.cellIndex) && td.cellIndex <= item.cellIndex && td.parentElement.rowIndex > 1 && `${td.dataset.value}` !== ""),
                    expenses_value = td.filter((_, td_expenses) => td_expenses.cellIndex === 3 && td.filter((_, td) => td.cellIndex === item.cellIndex && td.parentElement.rowIndex > 1 && `${td.dataset.value}` !== "").map((_, item) => item.parentElement.rowIndex).toArray().indexOf(td_expenses.parentElement.rowIndex) > -1).map((_, item) => item.dataset.value).toArray().sum(),
                    value = col.map((_, item) => item.dataset.value).toArray().sum();
                $(item).text(render_int(value / (is_profit ? expenses_value / 100 : 1)));
            });
        };
