...
def calculate_turnover(df):
    ...
    def turnover_in(column_name):
        ...
        data_turnover = (
            np.zeros((len(subcategories) + 2, 7)).astype("int")
            if flag
            else np.zeros((len(subcategories), 7)).astype("int")
        )
        columns_turnover = [
            "Сегмент",
            "Анкет",
        ...
        if flag:
            ...
            if df_turnover.iloc[-2:, 1].sum() != 0:
                df_turnover_ca.iloc[0, 1] = (
                    round(
                        df_turnover.iloc[i + 1, 1] / df_turnover.iloc[-2:, 1].sum(), 2
                    )
                    * 100
                )
            else:
            ...
            if df.loc[:, "payment_amount"].sum() != 0:
                df_turnover_ca.iloc[0, 2] = round(
                    df_turnover.iloc[i + 1, 2]
                    / df.loc[:, "payment_amount"].sum()
                    * 100,
                    2,
                )
            else:
            ...
            if df_turnover.iloc[-2:, 1].sum() != 0:
                df_turnover_ca.iloc[1, 1] = (
                    round(
                        df_turnover.iloc[i + 2, 1] / df_turnover.iloc[-2:, 1].sum(), 2
                    )
                    * 100
                )
            else:
    ...
    def get_channels_groups(df):
        ...
        for el in [
            ...
            new_row = {
                ...
                "Оплат": temp_df_ankets_ca[
                    temp_df_ankets_ca["payment_amount"] != 0
                ].shape[0],
                "% Оплат": round(
                ...
                "Чек": round(
                    temp_df_ankets_ca["payment_amount"].sum()
                    / temp_df_ankets_ca[temp_df_ankets_ca["payment_amount"] != 0].shape[
                        0
                    ],
                    1,
                )
                if temp_df_ankets_ca[temp_df_ankets_ca["payment_amount"] != 0].shape[0]
                != 0
                else 0,
            }
