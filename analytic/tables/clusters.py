from analytic._database import get_target_audience, get_leads_data
import os
import numpy as np
import pandas as pd
...
def calculate_clusters(df):
    ...
    def clusterDataFrame(x):
        if xTrain01[xTrain01[:, all_categories_len] == x].shape[0] != 0:
            mX = np.mean(
                xTrain01[xTrain01[:, all_categories_len] == x], axis=0
    ...
    def clusterDataFrame_education(x):
        mX = np.mean(
            xTrain01[xTrain01[:, all_categories_len + 1] == x], axis=0
        )  # Считаем среднее значение по кластеру
        sX = np.sum(
            xTrain01[xTrain01[:, all_categories_len + 1] == x], axis=0
        )  # Считаем сумму значение по кластеру
        ...
        res_dict.update(
            {
                f"{num} попаданий в ЦА": {
                    "Датасет": cluster_,
                    "Размер кластера": xTrain01[
                        xTrain01[:, all_categories_len] == num
                    ].shape[0],
                }
            }
        )
