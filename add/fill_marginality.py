    ...
    for i in range(ca_payment_analytic.shape[0]):
        if ca_payment_analytic.values[i][1] in [
            ...
            leads.loc[
                ...
                & (
                    leads["в_какой_сфере_сейчас_работаете"]
                    == ca_payment_analytic.values[i][1]
                )
                & (
                    leads["ваш_средний_доход_в_месяц"]
                    == ca_payment_analytic.values[i][2]
                )
                & (
                    leads[
                        "рассматриваете_ли_в_перспективе_платное_обучение_профессии_разработчик_искусственного_интеллекта"
                    ]
                    == ca_payment_analytic.values[i][3]
                ),
                "turnover_on_lead",
        ...
        leads["Маржинальность"] = (
            leads["turnover_on_lead"] / (850 + 0.35 * leads["turnover_on_lead"]) - 1
        ) / 1 + (
            leads["turnover_on_lead"] / (850 + 0.35 * leads["turnover_on_lead"]) - 1
        )
    return leads
...
if __name__ == "__main__":
    df = load_sheet(
        spreadsheetId="1DuRrQ3XPT1HlY_nVEvD6jCW6DEbO-t0SRw9vEEso2N8",
        sheetName="Лист 1",
        majorDimension="ROWS",
        startRow=1,
    ...
    df.loc[
        df["в_какой_сфере_сейчас_работаете"]
